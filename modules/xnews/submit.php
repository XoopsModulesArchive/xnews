...
//If no access
if (!$gpermHandler->checkRight('nw_submit', $perm_itemid, $groups, $module_id)) {
...
$approveprivilege = 0;
if (is_object($xoopsUser) && $gpermHandler->checkRight('nw_approve', $perm_itemid, $groups, $module_id)) {
    $approveprivilege = 1;
}
...
if (isset($_POST['preview'])) {
    ...
    if ($approveprivilege && 'edit' == $_GET['op']) {
        ...
        if (nw_getmoduleoption('authoredit', NW_MODULE_DIR_NAME) && is_object($xoopsUser) && isset($_GET['storyid']) && ('edit' == $_GET['op'] || 'preview' == $_POST['op'] || 'post' == $_POST['op'])) {
            $storyid = 0;
...
switch ($op) {
    case 'edit':
        ...
        if (!$gpermHandler->checkRight('nw_view', $story->topicid(), $groups, $module_id)) {
            redirect_header(NW_MODULE_URL . '/index.php', 0, _NOPERM);
            exit();
        }
        ...
        if (!nw_is_admin_group()) {
            require_once NW_MODULE_PATH . '/include/storyform.inc.php';
        } else {
    ...
    case 'post':
        ...
        if ($result) {
            ...
            if ($allowupload) {
                ...
                if (!is_writable(NW_ATTACHED_FILES_PATH)) {
