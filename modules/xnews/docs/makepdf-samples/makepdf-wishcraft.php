...
if ($filescount > 0) {
    foreach ($filesarr as $onefile) {
        if (false !== strpos($onefile->getMimetype(), 'image')) {
            $mime        = 'image';
            $newsfiles[] = ['visitlink' => NW_ATTACHED_FILES_URL . '/' . $onefile->getDownloadname(), 'file_realname' => $onefile->getFileRealName(), 'file_mimetype' => $mime];
    ...
    if (count($sarray) > 0) {
        foreach ($sarray as $onearticle) {
            ...
            if (0 != $seo_enabled) {
                ...
                $GLOBALS['xoopsTpl']->append('summary', ['story_id' => $onearticle->storyid(), 'htmltitle' => $htmltitle, 'infotips' => $tooltips, 'story_title' => $storyTitle, 'story_hits' => $onearticle->counter(), 'story_published' => formatTimestamp($onearticle->published, $dateformat)]);
            } else {
                $GLOBALS['xoopsTpl']->append(
                    'summary',
                    ['story_id' => $onearticle->storyid(), 'htmltitle' => $htmltitle, 'infotips' => $tooltips, 'story_title' => $onearticle->title(), 'story_hits' => $onearticle->counter(), 'story_published' => formatTimestamp($onearticle->published, $dateformat)]
                );
            }
        }
    }
