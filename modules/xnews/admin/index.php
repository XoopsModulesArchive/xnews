...
require_once NW_MODULE_PATH . '/class/blacklist.php';
require_once NW_MODULE_PATH . '/class/registryfile.php';
...
function LaunchNewsletter()
{
    ...
    if (file_exists(NW_MODULE_PATH . '/language/' . $xoopsConfig['language'] . '/newsletter.php')) {
        require_once NW_MODULE_PATH . '/language/' . $xoopsConfig['language'] . '/newsletter.php';
    } else {
        require_once NW_MODULE_PATH . '/language/english/newsletter.php';
    }
    echo '<br>';
    $story           = new nw_NewsStory();
    $exportedstories = [];
    ...
    if (count($exportedstories)) {
        ...
        foreach ($exportedstories as $onestory) {
            ...
            $search_pattern  = ['%title%', '%uname%', '%created%', '%published%', '%expired%', '%hometext%', '%bodytext%', '%description%', '%keywords%', '%reads%', '%topicid%', '%topic_title%', '%comments%', '%rating%', '%votes%', '%publisher%', '%publisher_id%', '%link%'];
            $replace_pattern = [
                $onestory->title(),
                $onestory->uname(),
                ...
                $onestory->uname(),
                $onestory->uid(),
                NW_MODULE_URL . '/article.php?storyid=' . $onestory->storyid(),
            ];
...
function topicsmanager()
{
    ...
    if ($topic_id > 0) {        // Edit mode
        $groups_ids                       = $gpermHandler->getGroupIds('nw_approve', $topic_id, $xoopsModule->getVar('mid'));
    ...
    $groups_ids = [];
    if ($topic_id > 0) {        // Edit mode
        $groups_ids                      = $gpermHandler->getGroupIds('nw_submit', $topic_id, $xoopsModule->getVar('mid'));
        $groups_ids                      = array_values($groups_ids);
    ...
    $groups_ids = [];
    if ($topic_id > 0) {        // Edit mode
        $groups_ids                    = $gpermHandler->getGroupIds('nw_view', $topic_id, $xoopsModule->getVar('mid'));
        $groups_ids                    = array_values($groups_ids);
...
function modTopicS()
{
    ...
    if (isset($_POST['groups_news_can_approve'])) {
        foreach ($_POST['groups_news_can_approve'] as $onegroup_id) {
            $gpermHandler->addRight('nw_approve', $xt->topic_id(), $onegroup_id, $xoopsModule->getVar('mid'));
        }
    }

    if (isset($_POST['groups_news_can_submit'])) {
        foreach ($_POST['groups_news_can_submit'] as $onegroup_id) {
            $gpermHandler->addRight('nw_submit', $xt->topic_id(), $onegroup_id, $xoopsModule->getVar('mid'));
        }
    }

    if (isset($_POST['groups_news_can_view'])) {
        foreach ($_POST['groups_news_can_view'] as $onegroup_id) {
            $gpermHandler->addRight('nw_view', $xt->topic_id(), $onegroup_id, $xoopsModule->getVar('mid'));
        }
    }
...
function delTopic()
{
    ...
    if (!isset($_POST['ok'])) {
        xoops_cp_header();
        echo '<h2>' . _AM_NW_TOPICSMNGR . '</h2>';
        $xt = new XoopsTopic($xoopsDB->prefix('nw_topics'), (int)$_GET['topic_id']);
        xoops_confirm(['op' => 'delTopic', 'topic_id' => (int)$_GET['topic_id'], 'ok' => 1], 'index.php', _AM_NW_WAYSYWTDTTAL . '<br>' . $xt->topic_title('S'));
    } else {
        xoops_cp_header();
        $xt = new XoopsTopic($xoopsDB->prefix('nw_topics'), (int)$_POST['topic_id']);
        if (isset($_SESSION['items_count'])) {
            $_SESSION['items_count'] = -1;
        }
    ...
    if (is_array($clone_arr) && $totalclones) {
        ...
        while ($ok) {
            if ($tmpcpt < $totalclones) {
                ...
                $class  = ('even' == $class) ? 'odd' : 'even';
                $output .= "<tr class='" . $class . "'><td align='center'>" . $clone_arr[$tmpcpt]['clone_name'] . "</td><td align='center'>" . $clone_arr[$tmpcpt]['clone_dir'] . "</td><td align='center'>" . $clone_arr[$tmpcpt]['clone_subprefix'] . "</td><td align='center'>" . round(
...
function NewsClonerApply()
{
    ...
    if (!empty($_POST['clone_modulename'])) {
        ...
        $new_subprefix = 'nw' . $Auto_increment;
        ...
        if (!is_dir($newPath)) { //&& !$old_subprefix == $new_subprefix ) {
            ...
            $result = $xoopsDB->query(
                'INSERT INTO ' . $xoopsDB->prefix('news_clonerdata') . ' (clone_name, clone_dir, clone_version, clone_subprefix, clone_installed)' . " VALUES ('" . $new_modulename . "', '" . $new_dirname . "', '" . $module_version . "', '" . $new_subprefix . "', 0);"
            );
...
function CloneDelete()
{
    ...
    if (!isset($_GET['clone_id'])) {
        ...
        xoops_confirm(['op' => 'clonedeleteapply', 'clone_id' => $cloneid, 'ok' => 1, 'module_name' => $module_dirname], 'index.php', $message);
    }
}
...
switch ($op) {
    ...
    case 'newarticle':
        ...
        $approveprivilege = 1;
        require_once NW_MODULE_PATH . '/include/storyform.original.php';
        break;
    ...
    case 'edit':
        ...
        require_once NW_MODULE_PATH . '/submit.php';
        break;
