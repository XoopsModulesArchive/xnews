...
class nw_NewsStory extends XoopsStory
{
    ...
    {
        $sql   = 'SELECT s.*, t.* FROM ' . $this->table . ' s, ' . $this->db->prefix('nw_topics') . ' t WHERE (storyid=' . (int)$storyid . ') AND (s.topicid=t.topic_id)';
    ...
    public static function getByTopic($topicid, $limit = 0)
    {
        $ret    = [];
        $db     = XoopsDatabaseFactory::getDatabaseConnection();
        $sql    = 'SELECT * FROM ' . $db->prefix('nw_stories') . ' WHERE topicid=' . (int)$topicid . ' ORDER BY published DESC';
    ...
    public function prepare2show($filescount)
    {
        ...
        if (1 == $xoopsModuleConfig['authoredit'] && (is_object($xoopsUser) && $xoopsUser->getVar('uid') == $this->uid())) {
            $approveprivilege = 1;
        }
        if ($approveprivilege) {
     ...
     * @param string $topicslist
     * @param int    $usetopicsdef Should we also export topics definitions ?
     * @param        $tbltopics
    ...
    public function hometext($format = 'Show')
    {
        ...
        switch ($format) {
            ...
            case 'InForm':
                ...
                $hometext = htmlspecialchars($hometext, ENT_QUOTES | ENT_HTML5);
                break;
        }
        return $hometext;
    }
    ...
    public function bodytext($format = 'Show')
    {
        ...
        switch ($format) {
            ...
            case 'InForm':
                ...
                $bodytext = htmlspecialchars($bodytext, ENT_QUOTES | ENT_HTML5);
                break;
        }
        return $bodytext;
    }
    ...
    public function nw_stripeKey($xoops_key, $num = 7, $length = 32, $uu = 0)
    {
        ...
        for ($i = 0, $iMax = strlen($xoops_key); $i < $iMax; ++$i) {
            if ($i < $length) {
                $uu++;
                if ($uu == $strip) {
