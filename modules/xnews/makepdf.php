...
if (!isset($xoopsModule)) {
    ...
    $xoopsModule   = $moduleHandler->getByDirname(NW_MODULE_DIR_NAME);
}
...
$content = '';
$content .= '<b><i><u>' . $myts->undoHtmlSpecialChars($article->title()) . '</u></i></b><br><b>' . $myts->undoHtmlSpecialChars($article->topic_title()) . '</b><br>' . _POSTEDBY . ' : ' . $myts->undoHtmlSpecialChars($article->uname()) . '<br>' . _MA_NW_POSTEDON . ' ' . formatTimestamp(
        $article->published(),
        $dateformat
    ) . '<br><br><br>';
