...
require_once NW_MODULE_PATH . '/class/keyhighlighter.class.php';
...
if ('' != xoops_trim($bodytext)) {
    ...
    if ($story_pages > 1) {
        require_once NW_MODULE_PATH . '/include/pagenav.php';
        ...
        if (0 == $storypage) {
            $story['text'] .= '<br>' . nw_getmoduleoption('advertisement', NW_MODULE_DIR_NAME) . '<br>' . $articletext[$storypage];
        } else {
            $story['text'] = $articletext[$storypage];
        }
    } else {
        $story['text'] .= '<br>' . nw_getmoduleoption('advertisement', NW_MODULE_DIR_NAME) . '<br>' . $bodytext;
    }
}
// Publicité
...
if ($story['poster']) {
    ...
    $story['poster_url']       = '';
    $story['poster_from']      = '';
    if (3 != nw_getmoduleoption('displayname', NW_MODULE_DIR_NAME)) {
...
if (1 == nw_getmoduleoption('pdf_detect', NW_MODULE_DIR_NAME)) {
...
if ($filescount > 0) {
    foreach ($filesarr as $onefile) {
        if (false !== strpos($onefile->getMimetype(), 'image')) {
            ...
            $newsfiles[]      = ['visitlink' => NW_ATTACHED_FILES_URL . '/' . $onefile->getDownloadname(), 'file_realname' => $onefile->getFileRealName(), 'file_mimetype' => $mime, 'file_downloadname' => NW_ATTACHED_FILES_URL . '/' . $onefile->getDownloadname()];
            $newsimages       = ['visitlink' => NW_ATTACHED_FILES_URL . '/' . $onefile->getDownloadname(), 'file_realname' => $onefile->getFileRealName(), 'file_mimetype' => $mime, 'thumbname' => NW_ATTACHED_FILES_URL . '/thumb_' . $onefile->getDownloadname()];
            ...
            $newsfiles[]   = [
                ...
                'visitlink'         => NW_MODULE_URL . '/visit.php?fileid=' . $onefile->getFileid(),
                ...
                'file_downloadname' => NW_ATTACHED_FILES_URL . '/' . $onefile->getDownloadname(),
            ];
            $newspdf       = [
                'file_id'           => $onefile->getFileid(),
                'visitlink'         => NW_MODULE_URL . '/visit.php?fileid=' . $onefile->getFileid(),
...
$xoopsTpl->assign('xoops_pagetitle', $article->title() . $complement . ' - ' . $article->topic_title() . ' - ' . htmlspecialchars($xoopsModule->name(), ENT_QUOTES | ENT_HTML5));
    ...
    if (count($sarray) > 0) {
        foreach ($sarray as $onearticle) {
            ...
            if (0 != $seo_enabled) {
                ...
                $xoopsTpl->append('summary', ['story_id' => $onearticle->storyid(), 'htmltitle' => $htmltitle, 'infotips' => $tooltips, 'story_title' => $storyTitle, 'story_hits' => $onearticle->counter(), 'story_published' => formatTimestamp($onearticle->published, $dateformat)]);
            } else {
                $xoopsTpl->append('summary', ['story_id' => $onearticle->storyid(), 'htmltitle' => $htmltitle, 'infotips' => $tooltips, 'story_title' => $onearticle->title(), 'story_hits' => $onearticle->counter(), 'story_published' => formatTimestamp($onearticle->published, $dateformat)]);
            }
        }
    }
