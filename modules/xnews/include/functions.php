...
function nw_CreateMetaDatas($story = null)
{
    ...
    if (nw_getmoduleoption('sitenavbar', NW_MODULE_DIR_NAME)) {
        ...
        $content .= sprintf("<link rel=\"%s\" href=\"%s\">\n", htmlspecialchars(_MA_NW_SUBMITNEWS, ENT_QUOTES | ENT_HTML5), NW_MODULE_URL . '/submit.php');
...
function nw_make_infotips($text)
{
    ...
    if ($infotips > 0) {
        ...
        return htmlspecialchars(mb_substr(strip_tags($text), 0, $infotips, 'UTF-8'), ENT_QUOTES | ENT_HTML5);
    }
}
...
function nw_remove_accents($chain)
{
    ...
    //                 Tab     Space      !        "        #        %        &        '        (        )        ,        /        :        ;        <        =        >        ?        @        [        \        ]        ^        {        |        }        ~       .
    $pattern = ['/%09/', '/%20/', '/%21/', '/%22/', '/%23/', '/%25/', '/%26/', '/%27/', '/%28/', '/%29/', '/%2C/', '/%2F/', '/%3A/', '/%3B/', '/%3C/', '/%3D/', '/%3E/', '/%3F/', '/%40/', '/%5B/', '/%5C/', '/%5D/', '/%5E/', '/%7B/', '/%7C/', '/%7D/', '/%7E/', '/%39/', "/\./"];
    $rep_pat = ['-', '-', '', '', '', '-100', '', '-', '', '', '', '-', '', '', '', '-', '', '', '-at-', '', '-', '', '-', '', '-', '', '-', '-', ''];
...
function nw_seo_UrlGenerator($op, $id, $short_url = '')
{
    if (0 != nw_getmoduleoption('seo_enable', NW_MODULE_DIR_NAME)) {
        ...
        if (1 == nw_getmoduleoption('seo_enable', NW_MODULE_DIR_NAME)) {
            ...
            if ('' != nw_getmoduleoption('seo_path', NW_MODULE_DIR_NAME)) {
                ...
                if (0 == nw_getmoduleoption('seo_level', NW_MODULE_DIR_NAME)) {
        ...
        } elseif (2 == nw_getmoduleoption('seo_enable', NW_MODULE_DIR_NAME)) {
