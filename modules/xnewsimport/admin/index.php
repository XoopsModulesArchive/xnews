...
function TopicSelect(): void
{
    ...
    $from_import        = $_POST['importfrom'] ?? 'nonselected';
    $to_import_clone_id = $_POST['importto'] ?? 'nonselected';
    ...
    if (false !== $from_modtables && is_array($from_modtables)) {
        foreach ($from_modtables as $from_table) {
            ...
            //&& $from_import_dirname != 'news') {
            if (count($from_table_arr) > 0) {
    ...
    if (0 === $totalCat) {
        ...
        if (0 === $totalArticles) {
            ...
            $allTopics  = $xt->getAllTopics($table_name, false);
